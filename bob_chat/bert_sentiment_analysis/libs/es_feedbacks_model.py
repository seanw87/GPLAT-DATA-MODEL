...
import time, datetime, json, os, sys, io
import jieba
import pandas as pd
import numpy as np
...
def get_yesterday_feedbacks():
    ...
    #     item['_source']["desc"] = item['_source']["desc"].split('{"openid"')[0]
    ...
    #         item['_source']['keyws'] if "keyws" in item['_source'] else ""
    #     ]
...
def update_yesterday_feedbacks(result_list):
    ...
    for i, item in enumerate(esresult):
        keyws = esresult[i].pop()
        esresult[i].append(result_list[i])
        esresult[i].append(keyws)
...
if __name__ == '__main__':
    ...
    # path = "../data/bob/"
    # pd_all = pd.read_csv(os.path.join(path, "qiuqiu-report-2020.04.12"), sep='#', encoding='utf-8')
    # pd_all = pd_all.drop(columns=pd_all.columns[0])

    # print(pd_all.iloc[:, 0].values.tolist()[0])
    # print(pd_all.iloc[:, 1].values.tolist()[0])
    # print(pd_all.iloc[:, 2].values.tolist()[0])
    # print(pd_all.iloc[:, 3].values.tolist()[0])
    # print(pd_all.iloc[:, 4].values.tolist()[0])
    # print(pd_all.iloc[:, 5].values.tolist()[0])
    # print(pd_all.iloc[:, 6].values.tolist()[0])
    # print(pd_all.iloc[:, 7].values.tolist()[0])
    # print(pd_all.iloc[:, 8].values.tolist()[0])
    # print(pd_all.iloc[:, 9].values.tolist()[0])
    # print(pd_all.iloc[:, 10].values.tolist()[0])
    # print(pd_all.iloc[:, 11].values.tolist()[0])
    # print(pd_all.iloc[:, 12].values.tolist()[0])
    # print(pd_all.iloc[:, 13].values.tolist()[0])
    # print(pd_all.iloc[:, 14].values.tolist()[0])
    # print(pd_all.iloc[:, 15].values.tolist()[0])
    # print(pd_all.iloc[:, 16].values.tolist()[0])
    # print(pd_all.iloc[:, 17].values.tolist()[0])

    # print(pd_all.values)

    # es_index = get_esindex()
    #
    # for index in range(len(pd_all.iloc[:, 0].values.tolist())):
    #     print(str(pd_all.iloc[:, 8].values.tolist()[index]))
    #     body = {
    #         'bug_from': str(pd_all.iloc[:, 0].values.tolist()[index]),
    #         'bug_picture': str(pd_all.iloc[:, 1].values.tolist()[index]),
    #         'cardno': str(pd_all.iloc[:, 2].values.tolist()[index]),
    #         'contact': str(pd_all.iloc[:, 3].values.tolist()[index]),
    #         'desc': str(pd_all.iloc[:, 4].values.tolist()[index]),
    #         'device': str(pd_all.iloc[:, 5].values.tolist()[index]),
    #         'device_type': str(pd_all.iloc[:, 6].values.tolist()[index]),
    #         'game_version': str(pd_all.iloc[:, 7].values.tolist()[index]),
    #         'id': str(pd_all.iloc[:, 8].values.tolist()[index]),
    #         'ip': str(pd_all.iloc[:, 9].values.tolist()[index]),
    #         'location': str(pd_all.iloc[:, 10].values.tolist()[index]),
    #         'rechtime': str(pd_all.iloc[:, 11].values.tolist()[index]),
    #         'rechtype': str(pd_all.iloc[:, 12].values.tolist()[index]),
    #         'submit_time': str(pd_all.iloc[:, 13].values.tolist()[index]),
    #         'submit_user': str(pd_all.iloc[:, 14].values.tolist()[index]),
    #         'type': str(pd_all.iloc[:, 15].values.tolist()[index]),
    #         'uid': str(pd_all.iloc[:, 16].values.tolist()[index]),
    #         'zesc_seg': str(pd_all.iloc[:, 17].values.tolist()[index])
    #     }
    #     print(es.index(index=es_index, body=body, doc_type="report", id=pd_all.iloc[:, 8].values.tolist()[index]))