import os, datetime, glob, re, sys, base64, urllib.parse
import threading
...
def proc_linebyline(game_flag, hour, line,
                    FH_MODULE_TARGET,
                    FH_MODULE_DIR_CREATION_ABNORMAL,
                    FH_MODULE_UNMATCHED):
    modules = line.split('|')
    if len(modules) >= 3:
...
# ↑↑ 拆分为文件块处理
def proc_file_by_block(filename, hour, worker_id, num_workers, log_dir):
...
# ↑↑↑ 处理文件列表（多线程版本）
def proc_filelist(flist, hour, log_dir):
    global FWRITEHANDLE
    global DEBUG_DICT
...
# ↑↑↑ 处理文件列表（多进程版本）
def proc_filelist_minstance(flist, hour, log_dir):
    global FWRITEHANDLE
    global DEBUG_DICT
    global FH_MODULE_TARGET
    global FH_MODULE_DIR_CREATION_ABNORMAL
    ...
    for filepath in flist:
        ...
        workers_instances = []
        for worker_id in range(workers):
            p = Process(target=proc_file_by_block, args=(filename, hour, worker_id, workers, log_dir))
            p.start()
            workers_instances.append(p)
        for p in workers_instances:
            p.join()
...
if __name__ == '__main__':
    ...
    # 文件依次处理
    proc_filelist_minstance(flist, hour, log_dir)
