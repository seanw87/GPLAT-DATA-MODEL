...
def safe_readline(f, worker_id):
    ...
    while True:
        try:
            ...
            pos -= 1
            f.seek(pos)
            print("enter safe_readline: nicodeDecodeError, pos: {}".format(pos))
        else:
            print("worker: {} has to read file from intermediate and safe_readline finished".format(worker_id))
            return True
...
def proc_file_by_block(filename, hour, worker_id, num_workers):
    ...
    with open(os.path.join(TOTAL_LOG_DIR, filename), mode='rb') as f:
        ...
        if offset > 0:
            safe_readline(f, worker_id)  # drop first incomplete line
        ...
        while True:
            ...
            # process line by line(game_flag, hour, line)
            modules = line.split('|')
            if len(modules) >= 3:
            ...
            # gap = end - f.tell()
            # if gap <= 1000:
            #     print("worker_id: {}, gap:{},  f.tell: {}, end: {}".format(worker_id, gap, f.tell(), end))
            if f.tell() > end:
                break
