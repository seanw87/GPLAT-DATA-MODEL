...
sys.path.append('./socialContentParticipial/common')
import common
...
def literal_participial(data_df):
    ...
    for i, row in data_df.iterrows():
        ...
        for item in contents_arr:
            itm = item.strip()
            # 排除语音文件和特定无效信息
            if re.search('\.amr', itm) is not None or \
                    re.search('^[0-9]+$', itm) is not None or \
                    re.search('^.* 组队\([0-9]+/[0-9]+\) .*$', itm) is not None:
                continue
            item_participialed = common.jieba_cut(itm)
...
def refer(test_set, flag, test_raw):
    ...
    merge_pd = merge_pd[merge_pd['content_literal'].apply(lambda x: len(str(x).strip()) > 2 and str(x)!='nan')]

    merge_col_pd = merge_pd[["refer", "uid"]]
    merge_filtered_pd = merge_col_pd[(merge_col_pd.refer == 0)]
    merge_filtered_pd.to_csv('data/merged_filtered_' + flag + '.csv', sep=',', index=False, encoding='utf-8')
